...
class OutputGenerator:
    def __init__(self, response, fileName, forms, tables, res_dir):
    ...
    def _outputWords(self, page, p):
        ...
        FileHelper.writeCSV("./" + self.res_dir + "/" + "{}-page-{}-words.csv".format(self.fileName, p),
                            csvFieldNames, csvData)

    def _outputText(self, page, p):
        text = page.text
        FileHelper.writeToFile("./" + self.res_dir + "/" + "{}-page-{}-text.txt".format(self.fileName, p),
                               text)

        textInReadingOrder = page.getTextInReadingOrder()
        FileHelper.writeToFile(
            "./" + self.res_dir + "/" + "{}-page-{}-text-inreadingorder.txt".format(self.fileName, p),
            textInReadingOrder)
    ...
    def _outputForm(self, page, p):
        ...
        FileHelper.writeCSV("./" + self.res_dir + "/" + "{}-page-{}-forms.csv".format(self.fileName, p),
                            csvFieldNames, csvData)
    ...
    def _outputTable(self, page, p):
        ...
        FileHelper.writeCSVRaw(
            "./" + self.res_dir + "/" + "{}-page-{}-tables.csv".format(self.fileName, p), csvData)
    ...
    def _outputTablePretty(self, page, p, table_format='github'):
        for table_number, table in enumerate(page.tables):
            ...
            FileHelper.writeToFile(
                "./" + self.res_dir + "/" + "{}-page-{}-table-{}-tables-pretty.txt".format(
    ...
    def run(self):
        ...
        FileHelper.writeToFile("./" + self.res_dir + "/" + "{}-response.json".format(self.fileName),
        ...
        for page in self.document.pages:
            ...
            FileHelper.writeToFile(
                "./" + self.res_dir + "/" +  "{}-page-{}-response.json".format(self.fileName, p),
    ...
    def _generateInsightsPerDocument(self, page, p, insights, medicalInsights,
        ...
        if (insights):
            FileHelper.writeCSV(
                "./" + self.res_dir + "/" + "{}-page-{}-insights-sentiment.csv".format(self.fileName, p),
                ["Sentiment"], sentiment)
            FileHelper.writeCSV(
                "./" + self.res_dir + "/" + "{}-page-{}-insights-entities.csv".format(self.fileName, p),
                ["Type", "Text", "Score", "BeginOffset", "EndOffset"],
                entities)
            FileHelper.writeCSV(
                "./" + self.res_dir + "/" + "{}-page-{}-insights-syntax.csv".format(self.fileName, p), [
                    "PartOfSpeech-Tag", "PartOfSpeech-Score", "Text",
                    "BeginOffset", "EndOffset"
                ], syntax)
            FileHelper.writeCSV(
                "./" + self.res_dir + "/" + "{}-page-{}-insights-keyPhrases.csv".format(self.fileName, p),
                ["Text", "Score", "BeginOffset", "EndOffset"], keyPhrases)

        if (medicalInsights):
            FileHelper.writeCSV(
                "./" + self.res_dir + "/" + "{}-page-{}-medical-insights-entities.csv".format(
                    self.fileName, p), [
            ...
            FileHelper.writeToFile(
                "./" + self.res_dir + "/" + "{}-page-{}-medical-insights-phi.json".format(
                    self.fileName, p), json.dumps(phi))

        if (translate):
            FileHelper.writeToFile(
                "./" + self.res_dir + "/" + "{}-page-{}-text-translation.txt".format(self.fileName, p),
                translation)
